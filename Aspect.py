...
# where,
#
# > f<sub>x</sub> = (z<sub>3</sub> - z<sub>5</sub> + 2(z<sub>2</sub> - z<sub>6</sub>) + z<sub>1</sub> - z<sub>7</sub>) / 8 * &Deltax
#
#  and,
#
# > f<sub>y</sub> = (z<sub>7</sub> - z<sub>5</sub> + 2(z<sub>8</sub> - z<sub>4</sub>) + z<sub>1</sub> - z<sub>3</sub>) / 8 * &Deltay
#
# &Deltax and &Deltay are the grid resolutions in the x and y direction respectively
#
# # Reference
...
impl Aspect :
    pub fn new() -> Aspect :
        ...
        toolbox = "Geomorphometric Analysis".to_string()
        description = "Calculates an aspect raster from an input DEM.".to_string()
        ...
        sep: String = path::MAIN_SEPARATOR.to_string()
        p = format!(":", env::current_dir().unwrap().display())
        e = format!(":", env::current_exe().unwrap().display())
        mut short_exe = e
            .replace(&p, "")
            .replace(".exe", "")
            .replace(".", "")
...
impl WhiteboxTool for Aspect :
    ...
    fn run<'a>(
        ...
        mut input_file = String::new()
        mut output_file = String::new()
        mut z_factor = -1f64
        ...
        for i in 0..args.len() :
            mut arg = args[i].replace("\"", "")
            arg = arg.replace("\'", "")
            cmd = arg.split("=") # in case an equals sign was used
            vec = cmd.collect::<Vec<&str>>()
            mut keyval = false
            if vec.len() > 1 :
                keyval = true
            
            flag_val = vec[0].to_lowercase().replace("--", "-")
        ...
        input = Arc::new(Raster::new(&input_file, "r")?)
        ...
        eight_grid_res = input.configs.resolution_x * 8.0

        if input.is_in_geographic_coordinates() && z_factor < 0.0 :
            # calculate a new z-conversion factor
            mut mid_lat = (input.configs.north - input.configs.south) / 2.0
        ...
        mut output = Raster::initialize_using_file(&output_file, &input)
        if output.configs.data_type != DataType::F32 && output.configs.data_type != DataType::F64 :
            output.configs.data_type = DataType::F32
        

        #num_procs = num_cpus::get() as isize
        import multiprocessing
        num_procs= multiprocessing.cpu_count()
        (tx, rx) = mpsc::channel()

        for tid in range(0, num_procs):
        #for tid in 0..num_procs:
            ...
            thread::spawn(move ||
            {
                ...
                for row in (0..rows).filter(|r| r % num_procs == tid):
                    mut data = vec![nodata columns as usize]
        ...
        for row in 0..rows:
            data = rx.recv().expect("Error receiving data from thread.")
